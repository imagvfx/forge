...
func findUserSettings(tx *sql.Tx, ctx context.Context, find forge.UserSettingFinder) ([]*forge.UserSetting, error) {
	...
	for rows.Next() {
		...
		case "picked_property_input_size":
			err = json.Unmarshal([]byte(value), &s.PickedPropertyInputSize)
		case "quick_searches":
		...
		case "recent_paths":
			err = json.Unmarshal([]byte(value), &s.RecentPaths)
		...
		case "show_archived":
			err = json.Unmarshal([]byte(value), &s.ShowArchived)
		default:
...
// getUserSettingShowArchived internally used in findEntries/searchEntries to avoid infinite recursion
// due to 'pinned_paths_v2' user setting, which calls getEntryById that calls findEntries.
//
// TODO: I need to resolve the base reason. Maybe shouldn't call getEntry in findUserSetting.
func getUserSettingShowArchived(tx *sql.Tx, ctx context.Context, user string) (bool, error) {
	rows, err := tx.QueryContext(ctx, `
		SELECT
			user_settings.value
		FROM user_settings
		LEFT JOIN accessors ON user_settings.user_id = accessors.id
		WHERE accessors.name=? AND user_settings.key='show_archived'`,
		user,
	)
	if err != nil {
		return false, err
...
func updateUserSetting(tx *sql.Tx, ctx context.Context, upd forge.UserSettingUpdater) error {
	...
	case "picked_property_input_size":
		size, ok := upd.Value.([2]int)
		if !ok {
			return fmt.Errorf("invalid update value type for key: %v", upd.Key)
		}
		value, err = json.Marshal(size)
		if err != nil {
			return err
		}
	case "quick_searches":
	...
	case "recent_paths":
		path, ok := upd.Value.(forge.StringSliceArranger)
		if !ok {
			return fmt.Errorf("invalid update value type for key: %v", upd.Key)
		}
		key := func(a string) string { return a }
		paths := forge.Arrange(setting.RecentPaths, path.Value, path.Index, key, false)
		limit := 20
		if len(paths) > limit {
			paths = paths[:limit]
		}
	...
	case "show_archived":
		show, ok := upd.Value.(bool)
		if !ok {
			return fmt.Errorf("invalid update value type for key: %v", upd.Key)
		}
		value, err = json.Marshal(show)
		if err != nil {
			return err
		}
	default:
