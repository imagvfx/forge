window.onload = function() {
		...
		if (handle != null) {
			...
			let fn = function() {
				if (handle.classList.contains("statusSelector")) {
					...
					let err = toggleUpdatePropertyPopup(sel);
					if (err) {
						printErrorStatus(err);
						return;
					}
	...
	let excelDateString = function(d) {
		function pad(n) {
			if (n < 10) {
				return "0" + n.toString();
			}
	...
	let handleDownloadExcel = function(req, name) {
		if (req.status != 200) {
	...
	if (downloadAsExcelButton != null) {
		downloadAsExcelButton.onclick = function() {
			...
			req.onload = function() {
				...
				let d = new Date();
				let name = "forge-" + excelDateString(d) + suffix + ".xlsx";
				handleDownloadExcel(req, name);
			}
	...
	if (backupAsExcelButton != null) {
		backupAsExcelButton.onclick = function() {
			...
			req.onload = function() {
				let d = new Date();
				let name = "forge-" + excelDateString(d) + "-backup" + ".xlsx";
				handleDownloadExcel(req, name);
			}
			req.onerror = function(err) {
				printErrorStatus("network error occurred. please check whether the server is down.");
...
function toggleUpdatePropertyPopup(sel) {
	let forceProp = sel.classList.contains("infoTitle");
	let onWriting = false;
	let popup = document.querySelector("#updatePropertyPopup");
	let popupHandle = document.querySelector(".popupHandle");
	if (popupHandle) {
	...
	let lastPopupHandle = document.querySelector(".lastPopupHandle");
	if (sel == lastPopupHandle) {
		onWriting = true;
	} else {
		// refresh contents
		if (lastPopupHandle) {
			lastPopupHandle.classList.remove("lastPopupHandle");
		}
		sel.classList.add("popupHandle");
		sel.classList.add("lastPopupHandle");
